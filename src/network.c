...
void forward_network(network *netp,char *logdir)
{
#ifdef GPU
    if(netp->gpu_index >= 0){
        forward_network_gpu(netp,logdir);   
        return;
    }
#endif
    network net = *netp;
...
float train_network_datum(network *net,char *logdir)
{
    *net->seen += net->batch;
    net->train = 1;
    forward_network(net,logdir);
...
float train_network_sgd(network *net, data d, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        float err = train_network_datum(net,"dummy");
        sum += err;
    }
    return (float)sum/(n*batch);
}

float train_network(network *net, data d, char *logdir)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        float err = train_network_datum(net,logdir);
        sum += err;
    }
    return (float)sum/(n*batch);
}
...
void forward_network_gpu(network *netp, char *logdir)
{
    network net = *netp;
    cuda_set_device(net.gpu_index);
    ...
    for(i = 0; i < net.n; ++i){
        ...
        ct ++;
        if (l.type == YOLO){
            l.forward_gpu_yolo(l, net,logdir);
        }
        else
            l.forward_gpu(l, net);
...
void *train_thread(void *ptr)
{
    ...
    *args.err = train_network(args.net, args.d,"dummy");
    return 0;
}
...
float train_networks(network **nets, int n, data d, int interval, char *logdir)
{
    int i;
